...
public class DashboardFrame extends JFrame {
    ...
    JPanel buildSidebar() {
        ...
        nav.add(sideBtn("History Pasien", e -> showPanel("history", new HistoryPasienFrame())));

        JPanel bottom = new JPanel(new GridLayout(0, 1));
        bottom.setBackground(SIDEBAR);

        JButton btnLogout = sideBtn("Logout", e -> {
            dispose();
    ...
    sep.setForeground(new Color(0x334155)); // warna garis
...
JButton sideBtn(String text, java.awt.event.ActionListener action) {
    ...
    b.setAlignmentX(Component.LEFT_ALIGNMENT); // penting!
    b.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45)); // full lebar

    b.setBorder(BorderFactory.createEmptyBorder(12, 18, 12, 0));
...
btnRefresh.addActionListener(e -> showHome());

headerTop.add(heading, BorderLayout.WEST);
headerTop.add(btnRefresh, BorderLayout.EAST);

p.add(headerTop, BorderLayout.NORTH);
        heading.setFont(new Font("Segoe UI", Font.BOLD, 16));
        heading.setForeground(TEXT);
    ...
    JScrollPane buildTable() {
        String[] cols = {
                "Nama Pasien", "Nomor Kamar", "Kronis", "BPJS", "Tanggal Masuk", "Estimasi Keluar", "Sisa Hari"};
        ...
        try {
            ...
            ResultSet rs = conn.prepareStatement(
                    "SELECT p.nama_pasien, k.nama_kamar, p.level_kronis, p.bpjs, ri.tanggal_masuk, ri.tanggal_keluar " +
            ...
            while (rs.next()) {
                ...
                long sisa = ChronoUnit.DAYS.between(
                        LocalDate.now(),
                        d.toLocalDate()
                );
    ...
    int[] getStats() {
        ...
        try {
            ...
            rs = conn.prepareStatement(
                    "SELECT COUNT(*) FROM rawat_inap WHERE status='aktif'"
            ).executeQuery();
            ...
            rs = conn.prepareStatement(
                    "SELECT COUNT(*) FROM rawat_inap WHERE status='selesai' AND DATE(tanggal_keluar)=CURDATE()"
            ).executeQuery();
    ...
    void autoDischarge() {
        try {
            ...
            ResultSet rs = conn.prepareStatement(
                    "SELECT id_rawat, id_kamar FROM rawat_inap WHERE tanggal_keluar<=NOW() AND status='aktif'"
            ).executeQuery();

            while (rs.next()) {

                PreparedStatement p2 = conn.prepareStatement(
    "UPDATE rawat_inap SET status='selesai', tanggal_dikeluarkan=NOW() WHERE id_rawat=?"
);
                p2.setInt(1, rs.getInt("id_rawat"));
                p2.executeUpdate();

                PreparedStatement p3 = conn.prepareStatement(
                        "UPDATE kamar SET status_kamar='kosong' WHERE id_kamar=?"
                );
                p3.setInt(1, rs.getInt("id_kamar"));
                p3.executeUpdate();
            }
